...
function PinScreen({onVerify,savedPin}: any) {
  ...
  return (
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"linear-gradient(135deg,#003D33,#00695C,#00897B)",padding:16}}>
      <style>{`@keyframes shk{0%,100%{transform:translateX(0)}25%,75%{transform:translateX(-7px)}50%{transform:translateX(7px)}}`}</style>
  ...
  return (
    <div>
      ...
      <Card>
        ...
        {[{label:"2. PERAWAT Instrumen",key:"circulatingNurse"},{label:"3. Perawat Onloop",key:"onloopNurse"}].map(({label,key})=>(
          <LF key={key} label={label}>
            <div style={{position:"relative"}}>
              <select style={{...iStyle(key),appearance:"none",paddingRight:30,cursor:"pointer"} as React.CSSProperties} value={opForm[key]||""} onChange={(ev:any)=>setR(key,ev.target.value)}>
...
function ViewMonitoring({monitoringEntries,setMonitoringEntries,monitoringCfg,setMonitoringCfg,showToast}:any) {
    ...
    <div>
      ...
      {subTab==="unduh"&&(
        <div>
          ...
          <div style={{background:"#F8FAFC",borderRadius:10,padding:"12px 14px",fontSize:12,color:"#64748B",border:"1px solid #E2E8F0",lineHeight:1.7}}>
...
export default function App() {
  ...
  const [supaCfg,  setSupaCfg]  = useState<SupabaseConfig>(defaultSupaCfg);
  ...
  /* Dropbox realtime debounce backup */
  useEffect(()=>{
    if(!dbxCfg.realtimeBackup||!dbxCfg.token) return;
    if(dbxRtRef.current) clearTimeout(dbxRtRef.current);
    dbxRtRef.current=setTimeout(async()=>{
      const data={ops,archive,notifs,lemburPegawai,lemburData,monitoringEntries,monitoringCfg};
      const res=await dropboxUpload(dbxCfg,data);
      if(res.ok) setDbxCfg(p=>({...p,lastBackup:fNow()}));
    },3000);
    return()=>{ if(dbxRtRef.current) clearTimeout(dbxRtRef.current); };
  ...
  /* Supabase realtime backup debounce (like Dropbox realtime) */
  const supaRtRef = useRef<ReturnType<typeof setTimeout>|null>(null);
  useEffect(()=>{
    if(!supaCfg.realtimeBackup||!supaCfg.url||!supaCfg.anonKey) return;
    if(supaRtRef.current) clearTimeout(supaRtRef.current);
    supaRtRef.current=setTimeout(async()=>{
      const data={exportedAt:fNow(),ops,archive,notifs,lemburPegawai,lemburData,monitoringEntries,monitoringCfg};
      const res=await supabaseBackup(supaCfg,data);
      if(res.ok) setSupaCfg(p=>({...p,lastBackup:fNow()}));
    },5000);
    return()=>{ if(supaRtRef.current) clearTimeout(supaRtRef.current); };
  },[ops,lemburData,lemburPegawai,monitoringEntries,supaCfg.realtimeBackup]);
  ...
  const content = (
    <ErrorBoundary>
      ...
      {tab==="monitoring" && <ViewMonitoring monitoringEntries={monitoringEntries} setMonitoringEntries={setMonitoringEntries} monitoringCfg={monitoringCfg} setMonitoringCfg={setMonitoringCfg} showToast={showToast}/>}
